...
@Component({
  ...
  templateUrl: './eth-scan-result-page.component.html',
  styleUrls: ['./eth-scan-result-page.component.scss']
})
export class EthScanResultPageComponent implements OnInit, OnDestroy {

  public isLoadingPipeline = false;
  public pipeline: EthAddressScanPipeline = {};
  public isAddressTaintJobLoading = false;
  public addressTaintJobResultPageNo = 0;
  public addressTaintJobResultPageSize = 30;
  public addressTaintJobResultPage: EthFlowAddressTaintJobResultPage = {
    hasNextPage: false,
    records: [],
  };


  public ethFlowRiskGraph: EthFlowRiskGraph = {};
        ...
        linkDataArray: graph.edges
          ...
          .map((edge) => {
            return {
              ...
              text: cryptoPipe.transform(edge.amount, 'satoshi', 'eth') + ' ETH',
  ...
  getEdgeWidth(satoshi: number) {
    ...
    const eth = pipe.transform(satoshi, 'satoshi', 'eth');
    if (eth < 0.1) {
      return 5;
    }
    if (eth < 0.5) {
